...
public class BsPurchaseCB extends AbstractConditionBean {
     ...
     * <pre>
     * <span style="color: #0000C0">purchaseBhv</span>.selectEntity(<span style="color: #553000">cb</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     <span style="color: #553000">cb</span>.<span style="color: #CC4747">setupSelect_MemberLoginAsBizManyToOne()</span>; <span style="color: #3F7E5E">// ...().with[nested-relation]()</span>
     *     <span style="color: #553000">cb</span>.query().set...
     * }).alwaysPresent(<span style="color: #553000">purchase</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">purchase</span>.<span style="color: #CC4747">getMemberLoginAsBizManyToOne()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     * @return The set-upper of nested relation. {setupSelect...().with[nested-relation]} (NotNull)
     */
    ...
    public static class HpSpecification extends HpAbstractSpecification<PurchaseCQ> {
        ...
        public WhiteDateTermCB.HpSpecification specifyWhiteDateTermAsValid() {
            ...
            if (_whiteDateTermAsValid == null) {
                _whiteDateTermAsValid = new WhiteDateTermCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryWhiteDateTermAsValid()
                                    , () -> _qyCall.qy().queryWhiteDateTermAsValid())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _whiteDateTermAsValid.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryWhiteDateTermAsValid()
                      , () -> xsyncQyCall().qy().queryWhiteDateTermAsValid()));
                }
            }
